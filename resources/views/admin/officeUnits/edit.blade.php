...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.office-units.update", [$officeUnit->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="name_en">{{ trans('cruds.officeUnit.fields.name_en') }}</label>
                <input class="form-control {{ $errors->has('name_en') ? 'is-invalid' : '' }}" type="text" name="name_en" id="name_en" value="{{ old('name_en', $officeUnit->name_en) }}">
