...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.project-revenue-exams.update", [$projectRevenueExam->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('exam') ? 'is-invalid' : '' }}" name="exam_id" id="exam_id" required>
                    @foreach($exams as $id => $entry)
                        <option value="{{ $id }}" {{ (old('exam_id') ? old('exam_id') : $projectRevenueExam->exam->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('exam'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('exam_name_bn') ? 'is-invalid' : '' }}" type="text" name="exam_name_bn" id="exam_name_bn" value="{{ old('exam_name_bn', $projectRevenueExam->exam_name_bn) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('exam_name_en') ? 'is-invalid' : '' }}" type="text" name="exam_name_en" id="exam_name_en" value="{{ old('exam_name_en', $projectRevenueExam->exam_name_en) }}" required>
