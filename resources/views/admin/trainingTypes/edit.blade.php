...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.training-types.update", [$trainingType->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="name_en">{{ trans('cruds.trainingType.fields.name_en') }}</label>
                <input class="form-control {{ $errors->has('name_en') ? 'is-invalid' : '' }}" type="text" name="name_en" id="name_en" value="{{ old('name_en', $trainingType->name_en) }}">
