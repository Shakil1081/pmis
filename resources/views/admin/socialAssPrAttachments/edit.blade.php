...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.social-ass-pr-attachments.update", [$socialAssPrAttachment->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('degree_membership_organization') ? 'is-invalid' : '' }}" type="text" name="degree_membership_organization" id="degree_membership_organization" value="{{ old('degree_membership_organization', $socialAssPrAttachment->degree_membership_organization) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $socialAssPrAttachment->description) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('certificate_achievement') ? 'is-invalid' : '' }}" type="text" name="certificate_achievement" id="certificate_achievement" value="{{ old('certificate_achievement', $socialAssPrAttachment->certificate_achievement) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('employee') ? 'is-invalid' : '' }}" name="employee_id" id="employee_id" required>
                    @foreach($employees as $id => $entry)
                        <option value="{{ $id }}" {{ (old('employee_id') ? old('employee_id') : $socialAssPrAttachment->employee->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
