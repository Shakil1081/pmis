            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('read') ? 'is-invalid' : '' }}" name="read_id" id="read_id">
                    @foreach($reads as $id => $entry)
                        <option value="{{ $id }}" {{ (old('read_id') ? old('read_id') : $language->read->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('read'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('write') ? 'is-invalid' : '' }}" name="write_id" id="write_id">
                    @foreach($writes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('write_id') ? old('write_id') : $language->write->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('write'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('speak') ? 'is-invalid' : '' }}" name="speak_id" id="speak_id">
                    @foreach($speaks as $id => $entry)
                        <option value="{{ $id }}" {{ (old('speak_id') ? old('speak_id') : $language->speak->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('speak'))
                    <div class="invalid-feedback">
