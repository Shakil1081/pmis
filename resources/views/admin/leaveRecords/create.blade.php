...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.leaveRecord.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.leave-records.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('type_of_leave') ? 'is-invalid' : '' }}" name="type_of_leave_id" id="type_of_leave_id" required>
                    @foreach($type_of_leaves as $id => $entry)
                        <option value="{{ $id }}" {{ old('type_of_leave_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('leave_category') ? 'is-invalid' : '' }}" name="leave_category_id" id="leave_category_id" required>
                    @foreach($leave_categories as $id => $entry)
                        <option value="{{ $id }}" {{ old('leave_category_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
