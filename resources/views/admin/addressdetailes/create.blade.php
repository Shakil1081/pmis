...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.addressdetaile.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.addressdetailes.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('flat_house') ? 'is-invalid' : '' }}" type="text" name="flat_house" id="flat_house" value="{{ old('flat_house', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('thana_upazila') ? 'is-invalid' : '' }}" name="thana_upazila_id" id="thana_upazila_id" required>
                    @foreach($thana_upazilas as $id => $entry)
                        <option value="{{ $id }}" {{ old('thana_upazila_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
