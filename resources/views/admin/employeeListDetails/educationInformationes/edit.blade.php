...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.education-informationes.update", [$educationInformatione->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('name_of_exam') ? 'is-invalid' : '' }}" name="name_of_exam_id" id="name_of_exam_id" required>
                    @foreach($name_of_exams as $id => $entry)
                        <option value="{{ $id }}" {{ (old('name_of_exam_id') ? old('name_of_exam_id') : $educationInformatione->name_of_exam->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('employee') ? 'is-invalid' : '' }}" name="employee_id" id="employee_id" required>
                    @foreach($employees as $id => $entry)
                        <option value="{{ $id }}" {{ (old('employee_id') ? old('employee_id') : $educationInformatione->employee->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
