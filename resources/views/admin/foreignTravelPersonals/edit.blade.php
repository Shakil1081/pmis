            ...
            <form method="POST" action="{{ route('admin.foreign-travel-personals.update', [$foreignTravelPersonal->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title"
                        id="title" value="{{ old('title', $foreignTravelPersonal->title) }}">
                    @if ($errors->has('title'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('from_date') ? 'is-invalid' : '' }}" type="text"
                        name="from_date" id="from_date" value="{{ old('from_date', $foreignTravelPersonal->from_date) }}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('leave') ? 'is-invalid' : '' }}" name="leave_id"
                        ...
                        @foreach ($leaves as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('leave_id') ? old('leave_id') : $foreignTravelPersonal->leave->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('leave'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('employee') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($employees as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('employee_id') ? old('employee_id') : $foreignTravelPersonal->employee->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
