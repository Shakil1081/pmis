...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-LanguageProficiency">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <tr data-entry-id="{{ $languageProficiency->id }}">
                            ...
                            <td>
                                @can('language_proficiency_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.language-proficiencies.show', $languageProficiency->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('language_proficiency_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.language-proficiencies.edit', $languageProficiency->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('language_proficiency_delete')
                                    <form action="{{ route('admin.language-proficiencies.destroy', $languageProficiency->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
