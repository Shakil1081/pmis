        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.upazilas.update', [$upazila->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-select select2 {{ $errors->has('district') ? 'is-invalid' : '' }}" name="district_id"
                        id="district_id" required>
                        @foreach ($districts as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('district_id') ? old('district_id') : $upazila->district->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name_bn') ? 'is-invalid' : '' }}" type="text"
                        name="name_bn" id="name_bn" value="{{ old('name_bn', $upazila->name_bn) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name_en') ? 'is-invalid' : '' }}" type="text"
                        name="name_en" id="name_en" value="{{ old('name_en', $upazila->name_en) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('grocode') ? 'is-invalid' : '' }}" type="text"
                        name="grocode" id="grocode" value="{{ old('grocode', $upazila->grocode) }}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-select select2 {{ $errors->has('forest_state') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($forest_states as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('forest_state_id') ? old('forest_state_id') : $upazila->forest_state->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
