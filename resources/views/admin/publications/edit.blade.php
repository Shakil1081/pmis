    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.publications.update", [$publication->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('publication_type') ? 'is-invalid' : '' }}" name="publication_type" id="publication_type" required>
                    <option value disabled {{ old('publication_type', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\Publication::PUBLICATION_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('publication_type', $publication->publication_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('publication_media') ? 'is-invalid' : '' }}" type="text" name="publication_media" id="publication_media" value="{{ old('publication_media', $publication->publication_media) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('publication_date') ? 'is-invalid' : '' }}" type="text" name="publication_date" id="publication_date" value="{{ old('publication_date', $publication->publication_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('publication_link') ? 'is-invalid' : '' }}" type="text" name="publication_link" id="publication_link" value="{{ old('publication_link', $publication->publication_link) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $publication->description) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('employee') ? 'is-invalid' : '' }}" name="employee_id" id="employee_id">
                    @foreach($employees as $id => $entry)
                        <option value="{{ $id }}" {{ (old('employee_id') ? old('employee_id') : $publication->employee->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('admin.publications.storeCKEditorImages') }}', true);
