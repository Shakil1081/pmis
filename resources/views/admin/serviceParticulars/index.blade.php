...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-ServiceParticular">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <tr data-entry-id="{{ $serviceParticular->id }}">
                            ...
                            <td>
                                @can('service_particular_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.service-particulars.show', $serviceParticular->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('service_particular_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.service-particulars.edit', $serviceParticular->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('service_particular_delete')
                                    <form action="{{ route('admin.service-particulars.destroy', $serviceParticular->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-ServiceParticular:not(.ajaxTable)').DataTable({ buttons: dtButtons })
