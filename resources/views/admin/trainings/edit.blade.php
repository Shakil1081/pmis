    ...
    <div class="card">
        <div class="card-header">
            ...
            <br><strong class="text-dark classname">
                @if (app()->getLocale() === 'bn')
                    {{ trans('cruds.employeeList.fields.fullname_bn') }}: {{ $training->employee->fullname_bn }}
                    &nbsp;
                    {{ trans('cruds.employeeList.fields.employeeid') }}:{{ $training->employee->employeeid }}
                @else
                    {{ trans('cruds.employeeList.fields.fullname_en') }}: {{ $training->employee->fullname_en }}
                    &nbsp;
                    {{ trans('cruds.employeeList.fields.employeeid') }}:{{ $training->employee->employeeid }}
                @endif
            </strong>
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.trainings.update', [$training->id]) }}"
                ...
                <div class="row row-cols-3">
                    <x-hidden-input name="employee_id" value="{{ $training->employee->id }}" />
                    ...
                    <select class="form-select select2 {{ $errors->has('employee') ? 'is-invalid' : '' }}" name="employee_id"
                        ...
                        @foreach ($employees as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('employee_id') ? old('employee_id') : $training->employee->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select select2 {{ $errors->has('training_type') ? 'is-invalid' : '' }}"
                            name="training_type_id" id="training_type_id" required>
                            @foreach ($training_types as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('training_type_id') ? old('training_type_id') : $training->training_type->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('training_name') ? 'is-invalid' : '' }}" type="text"
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('institute_name') ? 'is-invalid' : '' }}" type="text"
                            ...
                            value="{{ old('institute_name', $training->institute_name) }}">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select select2 {{ $errors->has('country') ? 'is-invalid' : '' }}"
                            ...
                            @foreach ($countries as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('country_id') ? old('country_id') : $training->country->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control date {{ $errors->has('end_date') ? 'is-invalid' : '' }}" type="text"
                            name="end_date" id="end_date" value="{{ old('end_date', $training->end_date) }}">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('grade') ? 'is-invalid' : '' }}" type="text"
                            name="grade" id="grade" value="{{ old('grade', $training->grade) }}">
                        @if ($errors->has('grade'))
                            <div class="invalid-feedback">
                                {{ $errors->first('grade') }}
                            </div>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('position') ? 'is-invalid' : '' }}" type="text"
                            name="position" id="position" value="{{ old('position', $training->position) }}">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('location') ? 'is-invalid' : '' }}" type="text"
                            name="location" id="location" value="{{ old('location', $training->location) }}">
