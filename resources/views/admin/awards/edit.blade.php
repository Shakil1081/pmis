    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.awards.update", [$award->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $award->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ground_area') ? 'is-invalid' : '' }}" type="text" name="ground_area" id="ground_area" value="{{ old('ground_area', $award->ground_area) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $award->date) }}">
                @if($errors->has('date'))
                    <div class="invalid-feedback">
                        {{ $errors->first('date') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('employee') ? 'is-invalid' : '' }}" name="employee_id" id="employee_id">
                    @foreach($employees as $id => $entry)
                        <option value="{{ $id }}" {{ (old('employee_id') ? old('employee_id') : $award->employee->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
